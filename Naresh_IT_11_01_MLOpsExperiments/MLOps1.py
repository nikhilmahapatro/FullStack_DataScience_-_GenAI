...
with mlflow.start_run():
    ...
    sns.heatmap(lr_conf_matrix, annot=True, fmt="d", cmap="Blues", xticklabels=iris.target_names,
...
with mlflow.start_run():
    rf_model = RandomForestClassifier(n_estimators=10, max_depth=3, min_samples_split=10, random_state=0,
                                      criterion='entropy')
    ...
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    plt.figure(figsize=(6, 6))
    sns.heatmap(rf_conf_matrix, annot=True, fmt="d", cmap="Blues", xticklabels=iris.target_names,
                yticklabels=iris.target_names)
